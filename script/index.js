...
define("script/model", ["require", "exports", "resource/config"], function (require, exports, config_json_1) {
    ...
    (function (Model) {
        ...
        ;
        Model.addPoints = function (a, b) {
            return ({
                x: a.x + b.x,
                y: a.y + b.y,
            });
        };
        Model.subPoints = function (a, b) {
            return ({
                x: a.x - b.x,
                y: a.y - b.y,
            });
        };
        Model.mulPoint = function (a, b) {
            return ({
                x: a.x * b,
                y: a.y * b,
            });
        };
        Model.averagePoints = function (points) {
            return Model.mulPoint(points.reduce(Model.addPoints, { x: 0, y: 0, }), 1 / points.length);
        };
        ;
        ;
        ;
                ...
                for (var j = i + 1; j < circles.length; j++) {
                    ...
                    if (hasFusionPath(fusionStatus)) {
                        ...
                        if (0 < wireLength) // == "wired" === fusionStatus
                         {
                            var wireWidthAdjustRate = 1.2; // This adjustment is probably necessary because something is wrong somewhere, but I won't actively investigate the cause this time. (This isn't the only place where the animation isn't smooth.)
                            ...
                            var mp0a = model_2.Model.averagePoints([model_2.Model.mulPoint(model_2.Model.averagePoints([tp1, tp4]), wireWidthRate * wireWidthAdjustRate), model_2.Model.mulPoint(cp1, 2 - (wireWidthRate * wireWidthAdjustRate))]);
                            var mp1 = model_2.Model.addPoints(mp0a, model_2.Model.mulPoint(model_2.Model.subPoints(tp1, tp4), wireLengthRate * 0.5));
                            var cxp1 = model_2.Model.addPoints(mp0a, model_2.Model.mulPoint(model_2.Model.subPoints(tp1, tp4), (2 - wireWidthRate) / 4));
                            var cxp2 = model_2.Model.addPoints(mp0a, model_2.Model.mulPoint(model_2.Model.subPoints(tp4, tp1), (2 - wireWidthRate) / 4));
                            var mp2 = model_2.Model.addPoints(mp0a, model_2.Model.mulPoint(model_2.Model.subPoints(tp4, tp1), wireLengthRate * 0.5));
                            ...
                            var mp0b = model_2.Model.averagePoints([model_2.Model.mulPoint(model_2.Model.averagePoints([tp3, tp2]), wireWidthRate * wireWidthAdjustRate), model_2.Model.mulPoint(cp2, 2 - (wireWidthRate * wireWidthAdjustRate))]);
                            var mp3 = model_2.Model.addPoints(mp0b, model_2.Model.mulPoint(model_2.Model.subPoints(tp3, tp2), wireLengthRate * 0.5));
                            var cxp3 = model_2.Model.addPoints(mp0b, model_2.Model.mulPoint(model_2.Model.subPoints(tp3, tp2), (2 - wireWidthRate) / 4));
                            var cxp4 = model_2.Model.addPoints(mp0b, model_2.Model.mulPoint(model_2.Model.subPoints(tp2, tp3), (2 - wireWidthRate) / 4));
                            var mp4 = model_2.Model.addPoints(mp0b, model_2.Model.mulPoint(model_2.Model.subPoints(tp2, tp3), wireLengthRate * 0.5));
